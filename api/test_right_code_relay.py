"""Right Code 中转站图像生成接口的冒烟测试

目标地址:
    https://www.right.codes/draw/v1/images/generations

使用方法:
    uv run python test_right_code_relay.py
    uv run python test_right_code_relay.py --api-key sk-xxx
"""
...
# 默认配置
DEFAULT_BASE_URL = "https://www.right.codes/draw/v1"  # Right Code 中转站地址
DEFAULT_MODEL = "gpt-image-2"  # 默认使用的图像生成模型
DEFAULT_API_KEY = ""  # 通过 --api-key 或环境变量 RIGHT_CODE_API_KEY 提供
...
def parse_args() -> argparse.Namespace:
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="测试 Right Code 中转站的图像生成接口")
    parser.add_argument(
        "--base-url", default=os.getenv("RIGHT_CODE_BASE_URL", DEFAULT_BASE_URL)
    )
    parser.add_argument(
        "--api-key",
        default=(
            os.getenv("RIGHT_CODE_API_KEY")
            or os.getenv("OPENAI_API_KEY")
            or DEFAULT_API_KEY
        ),
    )
    parser.add_argument(
        "--model", default=os.getenv("RIGHT_CODE_MODEL", DEFAULT_MODEL)
    )
        ...
        if response.status_code >= 400:
            ...
            if response.status_code == 401 and not args.api_key:
                print(
                    ...
                    "请使用 --api-key 参数，或设置环境变量 RIGHT_CODE_API_KEY / OPENAI_API_KEY"
                )
            raise SystemExit(1)
